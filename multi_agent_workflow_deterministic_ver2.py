...
NEW UI REQUIREMENT (right-side panel):
- Add a right-side panel that displays issuer information derived from plan/issuers_index.json.
- For each item, show the period folder name (e.g. "000090266425003648") and a readable list of issuer names.
- Keep it sufficiently general: it must render any number of CIKs and any number of periods.
- It must show at least: period + issuers (you may optionally group by CIK).
- The issuer list can be truncated visually if huge, but do not drop the data from rendering logic; prefer collapsible/scrollable UI.

You are given the issuers_index.json content below; you may embed it directly into the page (e.g., a <script type="application/json"> blob)
...
def validate_frontend_outputs() -> None:
    ...
    # NEW: issuer panel evidence (deterministic, lightweight)
...
Roles:
- Project Manager: requirements, tests, and agent task breakdown.
- Designer: create a one-page UI/UX spec and basic wireframe.
- Data Scanner: scan Unified_ver1/data and write plan/issuers_index.json.
- Frontend Developer: implement the page and game logic, plus issuer right-side panel.
